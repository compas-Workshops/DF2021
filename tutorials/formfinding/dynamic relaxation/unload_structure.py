...
proxy = Proxy('compas_fofin.analysis')
unload = proxy.unload_proxy
cablenetdata = unload(cablenet.to_data())
...
artist.clear_layer()
artist.draw_vertexlabels(color={key: (0.0, 0.0, 0.0) for key in cablenet.vertices_where({'is_anchor': True})})
artist.draw_edges(edges=edges,  color=edgecolor)
