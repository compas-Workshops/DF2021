...
from compas.geometry import project_points_plane, normalize_vector, scale_vector, subtract_vectors
...
for (u, v) in edges:
    ...
    if mesh.vertex_attribute(v, 'beam_pt') is None:
        if mesh.vertex_degree(v) == 2 and mesh.vertex_degree(u) == 3\
        or (mesh.vertex_degree(v) == 3 and mesh.vertex_degree(u) == 4):
            rx, ry, rz = mesh.vertex_attributes(u, ('rx', 'ry', 'rz'))
        else:
            rx, ry, rz = mesh.vertex_attributes(v, ('rx', 'ry', 'rz'))
        residual = scale_vector(normalize_vector([rx, ry, rz]), knit_beam_dis)
